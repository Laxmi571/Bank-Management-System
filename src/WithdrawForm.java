...
class WithdrawForm extends JPanel
{
    int acc;
    int bal;
    Connection conn;
    ...
    Statement stmt;
    int newBalance;
    WithdrawForm() throws SQLException
    {
       ...
       add(list);
        ...
        amount.addActionListener((e -> {
            ...
            newBalance=(bal-Integer.parseInt(amount.getText()));
            String insert="UPDATE `holder's info` SET `Balance`= "+newBalance+" WHERE `Account Number`= "+acc;
            balanceOutput.setText(newBalance+"");
            System.out.println("nnnnnnnnnn="+newBalance);
...
void setTransactions() throws SQLException {
    String transactionSql="insert into `transactions` values("+acc+","+bal+","+newBalance+","+1+")";
    System.out.println(transactionSql);
    Statement stmt2=conn.createStatement();
    stmt2.executeUpdate(transactionSql);
